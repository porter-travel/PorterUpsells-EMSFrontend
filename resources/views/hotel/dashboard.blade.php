    ...
    <div class="px-4 pt-6">
        <div class="flex items-end justify-start mb-6 flex-wrap">
            ...
            <div class="md:basis-5/12 basis-full md:pl-12 pt-4 md:pt-0">
                <p class="text-black open-sans font-semibold md:text-2xl mb-6">Hi {{($data['name'])}},</p>
                <div class="flex items-start justify-start mb-3">
                    ...
                    <p class="open-sans md:text-xl pl-2 text-[#5A5A5A]">It’s just {{$data['days_until_arrival']}} days until you arrive!</p>
                </div>
                <div class="flex items-start justify-start mb-3">
<div>
                <svg class="mr-2 mt-1" width="22" height="22" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <p class="open-sans md:text-xl pl-2 text-black">To personalise your stay, select from the options below.</p>
                </div>
            </div>
        </div>
        <hr>
            ...
            @foreach($products as $product)
                <a href="/hotel/{{$hotel->id}}/item/{{$product->id}}"
                   class="flex items-start justify-between flex-col basis-1/2 md:basis-1/3 lg:basis-1/4 w-1/2 px-4 mb-4">
                    ...
                    <div>
                        <h3 class="open-sans text-sm md:text-xl">{{$product->name}}</h3>
                        <p class="open-sans text-sm md:text-xl font-semibold"><strong>£{{App\Helpers\Money::addTaxAndFormat($product->price)}}</strong></p>
                    </div>
