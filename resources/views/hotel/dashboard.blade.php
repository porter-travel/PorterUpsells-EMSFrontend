<x-guest-layout>
    <div class="px-4 pt-6">
        <div class="flex items-end justify-start mb-6">
            <div
                class="basis-7/12 h-[250px] relative rounded p-4 flex flex-col"
            ...
            <div class="basis-5/12 pl-12">
                <p class="text-black open-sans font-semibold text-2xl mb-6">Hi {{($data['name'])}},</p>
                <div class="flex items-start justify-start mb-3">
                    <svg class="mr-2 mt-1" width="22" height="22" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <circle cx="11" cy="11" r="10" fill="#C7EDF2" stroke="#C7EDF2" stroke-width="2"/>
                        <path d="M5.78906 11.543L8.53906 14.293L14.7266 8.10547" stroke="#5A5A5A" stroke-width="2" stroke-linecap="round"/>
                    </svg>

                    <p class="open-sans text-xl pl-2 text-[#5A5A5A]">It’s just {{$data['days_until_arrival']}} days until you arrive!</p>
                </div>
                <div class="flex items-start justify-start mb-3">
                ...
                <p class="open-sans text-xl pl-2 text-black">To personalise your stay, select from the options below.</p>
                </div>
            </div>
        </div>
        <hr>
                ...
                <a class="flex items-center justify-between relative" href="/hotel/{{$hotel->id}}/cart"
                   class="text-black">View
                    ...
                    @if($cart && $cart['cartCount'] > 0)
                        <span
                            class="bg-teal text-black rounded-full px-2 py-1 ml-2 absolute right-0 top-0 translate-x-full -translate-y-1/2 text-xs">{{$cart['cartCount']}}</span>
                    @endif
                </a>
            </div>
        </div>
            ...
            @foreach($products as $product)
                <a href="/hotel/{{$hotel->id}}/item/{{$product->id}}"
                   class="flex items-start justify-between flex-col basis-1/2 md:basis-1/3 lg:basis-1/4 w-1/2 px-4 mb-4">
                    ...
                    <div>
                        <h3 class="open-sans text-xl">{{$product->name}}</h3>
                        <p class="open-sans text-xl font-semibold"><strong>£{{App\Helpers\Money::addTaxAndFormat($product->price)}}</strong></p>
                    </div>
